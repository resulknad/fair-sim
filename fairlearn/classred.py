...
class _Lagrangian:
    ...
    def best_h(self, lambda_vec):
        ...
        h_proba = lambda X: list(map(lambda x: x[1],classifier.predict_proba(X)))
        ...
        if h_val < best_val-_PRECISION:
            ...
            h_idx = len(self.hs)
            self.hs.at[h_idx] = h
            self.hs_proba.at[h_idx] = h_proba
...
def expgrad(dataX, dataA, dataY, learner, cons=moments.DP(), eps=0.01,
    ...
    weights = Qs[best_t]
    hs = lagr.hs
    hs_proba = lagr.hs_proba
    assert(len(hs)==len(hs_proba))
    for h_idx in hs.index:
        if not weights.index.contains(h_idx):
            weights.at[h_idx] = 0.0
    best_classifier = lambda X: _mean_pred(X, hs_proba, weights)
